...
class AzureDeployImpl extends baseDeployImpl_1.BaseDeployImpl {
    ...
    async deploy(args) {
        ...
        const resourceId = (0, common_1.checkMissingArgs)("resourceId", args.resourceId);
        const azureResource = this.parseResourceId(resourceId);
        const azureCredential = await (0, azureResourceOperation_1.getAzureAccountCredential)(this.context.azureAccountProvider);
    ...
    async cleanup() {
        if (this.zipFilePath && !this.dryRun && fs.existsSync(this.zipFilePath)) {
            try {
                ...
                const parentFolder = path_1.default.dirname(this.zipFilePath);
                if ((await fs.readdir(parentFolder)).length === 0) {
                    await fs.remove(parentFolder);
                }
            }
            catch (e) {
                this.logger.warning(`Failed to remove zip package. ${JSON.stringify(e, Object.getOwnPropertyNames(e))}`);
            }
        }
    }
    /**
    ...
    parseResourceId(resourceId) {
        return (0, azureResourceOperation_1.parseAzureResourceId)(resourceId, this.pattern);
    }
    /**
        ...
        for (let i = 0; i < deployConstant_1.DeployConstant.DEPLOY_CHECK_RETRY_TIMES; ++i) {
            ...
            if (res) {
                if ((res === null || res === void 0 ? void 0 : res.status) === commonConstant_1.HttpStatusCode.ACCEPTED) {
                    await (0, utils_1.waitSeconds)(deployConstant_1.DeployConstant.BACKOFF_TIME_S);
                }
                    ...
                    if (((_g = res.data) === null || _g === void 0 ? void 0 : _g.status) === deployConstant_1.DeployStatus.Failed) {
                        this.logger.warning((0, localizeUtils_1.getDefaultString)("error.deploy.DeployRemoteStartError", location, JSON.stringify(res.data)));
                    }
                    return res.data;
                }
                else {
    ...
    async createAzureDeployConfig(azureResource, azureCredential) {
        ...
        try {
            const defaultScope = `${(0, constants_1.getResourceServiceEndpoint)(constants_1.ResourceServiceType.Azure)}/.default`;
        ...
        const listResponse = await (0, azureSdkErrorHandler_1.wrapAzureOperation)(() => managementClient.webApps.beginListPublishingCredentialsAndWait(azureResource.resourceGroupName, azureResource.instanceId), (e) => new error_1.GetPublishingCredentialsError(azureResource.instanceId, azureResource.resourceGroupName, e, this.helpLink), (e) => new error_1.GetPublishingCredentialsError(azureResource.instanceId, azureResource.resourceGroupName, e, this.helpLink));
    ...
    async restartFunctionApp(azureResource) {
        ...
        try {
            ...
            this.logger.warning((0, localizeUtils_1.getLocalizedString)("driver.deploy.error.restartWebAppError"));
        }
    }
}
