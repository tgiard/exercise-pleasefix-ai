...
(function (DotnetVersion) {
    ...
    DotnetVersion["v70"] = "7.0";
    DotnetVersion["v80"] = "8.0";
    DotnetVersion["v90"] = "9.0";
    DotnetVersion["v10"] = "10.0";
...
class DotnetChecker {
    ...
    async install() {
        ...
        if (!(await this.validate())) {
            ...
            throw new error_1.DepsCheckerError({
                default: (0, localizeUtils_1.getDefaultString)("error.common.InstallSoftwareError", installedNameWithVersion),
                localized: (0, localizeUtils_1.getLocalizedString)("error.common.InstallSoftwareError", installedNameWithVersion),
            }, helpLink_1.dotnetFailToInstallHelpLink);
        }
        this._telemetry.sendEvent(telemetry_1.DepsCheckerEvent.dotnetInstallCompleted);
    }
    async command() {
    ...
    async runDotnetInstallScript(version, installDir) {
        ...
        try {
            ...
            this._logger.debug(`Finished running dotnet-install script, command = '${command.join(" ")}', options = '${JSON.stringify(options)}', stdout = '${String(stdout)}', stderr = '${String(stderr)}'`);
            const timecost = Number(((perf_hooks_1.performance.now() - start) / 1000).toFixed(2));
            if (stderr && stderr.length > 0) {
                const errorMessage = `${(0, localizeUtils_1.getLocalizedString)("error.common.InstallSoftwareError", installedNameWithVersion)} ${message_1.Messages.dotnetInstallStderr()} stdout = '${String(stdout)}', stderr = '${String(stderr)}', timecost = '${timecost}s'`;
            ...
            const errorMessage = `${(0, localizeUtils_1.getLocalizedString)("error.common.InstallSoftwareError", installedNameWithVersion)} ${message_1.Messages.dotnetInstallErrorCode()}, ` +
                `command = '${command.join(" ")}', options = '${JSON.stringify(options)}', error = '${String(error)}', stdout = '${String(error.stdout)}', stderr = '${String(error.stderr)}', timecost = '${timecost}s'`;
            this._telemetry.sendSystemErrorEvent(telemetry_1.DepsCheckerEvent.dotnetInstallScriptError, telemetry_1.TelemtryMessages.failedToExecDotnetScript, errorMessage);
