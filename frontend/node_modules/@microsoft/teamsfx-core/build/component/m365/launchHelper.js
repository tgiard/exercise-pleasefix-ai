...
class LaunchHelper {
        ...
        switch (hub) {
            ...
            case constants_2.HubTypes.outlook: {
                ...
                const baseUrl = hasCopilotExtensionOnly
                    ? `${constants_4.outlookBaseUrl}/host/${constants_4.outlookCopilotAppId}`
                    : capabilities.includes("staticTab")
                        ? `${constants_4.outlookBaseUrl}/host/${result.value}`
                        : `${constants_4.outlookBaseUrl}/mail`;
                url = new URL(baseUrl);
                break;
            }
            case constants_2.HubTypes.office:
                {
                    ...
                    const baseUrl = hasCopilotExtensionOnly
                        ? `${constants_4.officeBaseUrl}/chat?auth=2`
                        : `${constants_4.officeBaseUrl}/m365apps/${result.value}?auth=2`;
                    url = new URL(baseUrl);
                }
                break;
        }
    ...
    async getTidFromToken() {
        try {
            const statusRes = await this.m365TokenProvider.getStatus({ scopes: (0, constants_1.AppStudioScopes)() });
