...
async function getTemplateInfosFromApiSpec(context, inputs, projectType, actionContext) {
    ...
    const openapiSpecPath = normalizePath(path_1.default.join(teamsfx_api_1.AppPackageFolderName, teamsfx_api_1.DefaultApiSpecFolderName, openapiSpecFileName));
    const authData = inputs.apiAuthData;
    const convertedAuthData = [];
    if (authData && authData.length > 0) {
        for (const auth of authData) {
            const envName = (_a = auth.registrationId) !== null && _a !== void 0 ? _a : (0, helper_1.getEnvName)((_b = auth.authName) !== null && _b !== void 0 ? _b : "");
            convertedAuthData.push({
                authName: (_c = auth.authName) !== null && _c !== void 0 ? _c : "",
    ...
    context.telemetryReporter.sendTelemetryEvent(const_1.declarativeAgentExistingApiSpecUrlTelemetryEvent, {
        ...
        ["auth-type" /* telemetryProperties.authType */]: (_d = authData === null || authData === void 0 ? void 0 : authData.map((item) => item.authType).join(",")) !== null && _d !== void 0 ? _d : "None",
    });
...
async function generateFilesFromApiSpec(context, inputs, destinationPath, projectType, componentName) {
    ...
    const apiSpecFolderPath = path_1.default.join(destinationPath, teamsfx_api_1.AppPackageFolderName, teamsfx_api_1.DefaultApiSpecFolderName);
    ...
    const pluginManifestPath = templateState.type === m365_spec_parser_1.ProjectType.Copilot
        ? path_1.default.join(destinationPath, teamsfx_api_1.AppPackageFolderName, teamsfx_api_1.DefaultPluginManifestFileName)
        : undefined;
