...
exports.manifestUtils = exports.ManifestUtils = exports.SharePointAppId = void 0;
...
class ManifestUtils {
            ...
            switch (capability.name) {
                case "staticTab":
                    ...
                    if (capability.snippet) {
                        ...
                        if (capability.existingApp) {
                            const template = (0, lodash_1.cloneDeep)(constants_2.STATIC_TABS_TPL_EXISTING_APP[0]);
                            // eslint-disable-next-line @typescript-eslint/restrict-plus-operands
                            template.entityId = "index" + staticTabIndex;
                            appManifest.staticTabs.push(template);
                        }
                        else {
                            const tabManifest = inputs.features === CapabilityOptions_1.TabCapabilityOptions.dashboardTab().id
                ...
                case "configurableTab":
                    ...
                    if (capability.snippet) {
                        ...
                        if (capability.existingApp) {
                            appManifest.configurableTabs = appManifest.configurableTabs.concat((0, constants_2.getConfigurableTabsTplExistingAppBasedOnVersion)(manifestVersion));
                        }
                        else {
                            appManifest.configurableTabs = appManifest.configurableTabs.concat((0, constants_2.getConfigurableTabsTplBasedOnVersion)(manifestVersion));
                        }
                    }
                    break;
                case "Bot":
                    ...
                    if (capability.snippet) {
                        ...
                        if (capability.existingApp) {
                            appManifest.bots = appManifest.bots.concat((0, constants_2.getBotsTplExistingAppBasedOnVersion)(manifestVersion));
                        }
                        else {
                            ...
                            if (inputs.features) {
                                ...
                                if (feature === CapabilityOptions_1.BotCapabilityOptions.commandBot().id ||
                                    feature == CapabilityOptions_1.BotCapabilityOptions.workflowBot().id) {
                                    // command and response bot or workflow bot
                                    appManifest.bots = appManifest.bots.concat((0, constants_2.getBotsTplForCommandAndResponseBasedOnVersion)(manifestVersion));
                                }
                                else if (feature === CapabilityOptions_1.BotCapabilityOptions.notificationBot().id) {
                                    // notification
                                    appManifest.bots = appManifest.bots.concat((0, constants_2.getBotsTplForNotificationBasedOnVersion)(manifestVersion));
                                }
                                else {
                                    // legacy bot
                                    appManifest.bots = appManifest.bots.concat((0, constants_2.getBotsTplBasedOnVersion)(manifestVersion));
                                }
                            }
    ...
    async getManifestV3(manifestTemplatePath, context, generateIdIfNotResolved = true) {
        ...
        try {
            if (!(await fs_extra_1.default.pathExists(manifestTemplatePath))) {
                return (0, teamsfx_api_1.err)(new common_1.FileNotFoundError("ManifestUtils", manifestTemplatePath));
            }
            manifest = await teamsfx_api_1.AppManifestUtils.readTeamsManifest(manifestTemplatePath);
        }
        catch (error) {
            return (0, teamsfx_api_1.err)(new common_1.JSONSyntaxError(manifestTemplatePath, error, "ManifestUtils"));
        }
        let teamsAppId = "";
    ...
    extractManifestFromArchivedFile(archivedFile) {
        ...
        if (!manifestFile) {
            return (0, teamsfx_api_1.err)(results_1.AppStudioResultFactory.UserError(errors_1.AppStudioError.FileNotFoundError.name, errors_1.AppStudioError.FileNotFoundError.message(constants_2.Constants.MANIFEST_FILE)));
        }
    ...
    async resolveLocFile(locFilePath) {
        if (!(await fs_extra_1.default.pathExists(locFilePath))) {
            return (0, teamsfx_api_1.err)(new common_1.FileNotFoundError("teamsApp", locFilePath));
        }
        const locFileString = await fs_extra_1.default.readFile(locFilePath, "utf8");
        const resolvedLocFileString = (0, common_2.expandEnvironmentVariable)(locFileString);
        const unresolvedEnvVariables = (0, common_2.getEnvironmentVariables)(resolvedLocFileString);
        if (unresolvedEnvVariables && unresolvedEnvVariables.length > 0) {
            return (0, teamsfx_api_1.err)(new common_1.MissingEnvironmentVariablesError("teamsApp", unresolvedEnvVariables.join(","), locFilePath));
        }
        return (0, teamsfx_api_1.ok)(resolvedLocFileString);
    }
    /**
     * Parse the manifest and get properties
     * @param manifest
     */
    parseCommonProperties(manifest) {
        ...
        if (manifest.webApplicationInfo &&
            ...
            manifest.webApplicationInfo.id == exports.SharePointAppId) {
        ...
        if (manifest.composeExtensions &&
            manifest.composeExtensions.length > 0 &&
            manifest.composeExtensions[0].composeExtensionType == "apiBased" &&
            ((_a = manifest.composeExtensions[0].authorization) === null || _a === void 0 ? void 0 : _a.authType) == "microsoftEntra") {
     ...
     */
    parseCommonTelemetryProperties(manifest) {
