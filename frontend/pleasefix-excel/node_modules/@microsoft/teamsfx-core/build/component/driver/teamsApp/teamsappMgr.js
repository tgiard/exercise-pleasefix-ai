...
class TeamsAppMgr {
    async ensureAppPackageFile(inputs) {
        ...
        if (!inputs["package-file"]) {
            ...
            if (packageRes.isErr()) {
                return (0, teamsfx_api_1.err)(packageRes.error);
            }
            inputs["package-file"] = packageRes.value.outputZipPath;
        }
        if (!(await fs_extra_1.default.pathExists(inputs["package-file"]))) {
            return (0, teamsfx_api_1.err)(new common_1.FileNotFoundError("TeamsAppMgr", inputs["package-file"]));
        }
    ...
    async readManifestFromZip(appPackagePath) {
        ...
        return (0, teamsfx_api_1.err)(new common_1.FileNotFoundError("TeamsAppMgr", appPackagePath + ":" + constants_2.Constants.MANIFEST_FILE));
    }
    async checkAndTryToLoadEnv(inputs) {
        ...
        const unresolved = [];
        const resovled = [];
        (0, lifecycle_1.resolveString)(manifestString, resovled, unresolved);
        let env = undefined;
            ...
            if (!inputs["env-file"]) {
                ...
                if (inputs.env) {
                    ...
                    if (envs.includes(inputs.env)) {
                        ...
                        return (0, teamsfx_api_1.err)(new common_1.FileNotFoundError("TeamsAppMgr", path.join(envFolder, `.env.${inputs.env}`)));
                    }
                }
                else {
                    //env not provided
                    if (envs.length > 1) {
                        //need provide
                        return (0, teamsfx_api_1.err)(new common_1.MissingRequiredInputError("env", "TeamsAppMgr"));
                    }
    ...
    async packageTeamsApp(inputs) {
        if (!inputs["manifest-file"]) {
            ...
            if (!(await fs_extra_1.default.pathExists(defaultManifestPath))) {
                return (0, teamsfx_api_1.err)(new common_1.MissingRequiredInputError("package-file/manifest-file", "TeamsAppMgr"));
            }
            inputs["manifest-file"] = defaultManifestPath;
        }
        else {
            if (!(await fs_extra_1.default.pathExists(inputs["manifest-file"]))) {
                return (0, teamsfx_api_1.err)(new common_1.FileNotFoundError("TeamsAppMgr", inputs["manifest-file"]));
            }
        }
        const loadEnvRes = await this.checkAndTryToLoadEnv(inputs);
        if (loadEnvRes.isErr())
            return (0, teamsfx_api_1.err)(loadEnvRes.error);
        ...
        const driverContext = (0, utils_1.createDriverContext)(inputs);
        // For TSP projects
        await (0, tools_1.runForTypeSpecProject)(inputs.projectPath, driverContext);
        const res = (await buildDriver.execute(packageArgs, driverContext)).result;
        if (res.isErr()) {
            return (0, teamsfx_api_1.err)(res.error);
        }
        return (0, teamsfx_api_1.ok)(packageArgs);
    }
    /**
     * entry of validate teams app
     */
    async validateTeamsApp(inputs) {
        const context = (0, utils_1.createDriverContext)(inputs);
    ...
    async updateTeamsApp(inputs) {
        ...
        if (validateRes.isErr()) {
            return (0, teamsfx_api_1.err)(validateRes.error);
        }
        ...
        const url = util.format(constants_2.Constants.DEVELOPER_PORTAL_APP_PACKAGE_URL, teamsAppId, loginHint);
        const message = [
            {
    ...
    async publishTeamsApp(inputs) {
        ...
        void ((_a = driverContext.ui) === null || _a === void 0 ? void 0 : _a.showMessage("info", `publish successfully, Go to admin portal: ${constants_2.Constants.TEAMS_ADMIN_PORTAL}`, false));
        return (0, teamsfx_api_1.ok)(undefined);
    }
}
