...
let ValidateWithTestCasesDriver = class ValidateWithTestCasesDriver {
    ...
    async validate(args, context) {
        ...
        if (manifestFile) {
            ...
            const manifest = teamsfx_api_1.TeamsManifestConverter.jsonToManifest(manifestContent);
            metadataUtil_1.metadataUtil.parseManifest(manifest);
            // Add common properties like isCopilotPlugin: boolean
            const manifestTelemetries = ManifestUtils_1.manifestUtils.parseCommonTelemetryProperties(manifest);
                    ...
                    if (validation.status === AsyncAppValidationResponse_1.AsyncAppValidationStatus.InProgress ||
                        ...
                        if (context.platform === teamsfx_api_1.Platform.CLI) {
                            const message = [
                                ...
                                {
                                    content: `${(0, constants_1.getResourceServiceEndpoint)(constants_1.ResourceServiceType.TDP)}/apps/${manifest.id}/app-validation/${validation.id}`,
                            ...
                            const message = (0, localizeUtils_1.getLocalizedString)("driver.teamsApp.progressBar.validateWithTestCases.conflict", `${(0, constants_1.getResourceServiceEndpoint)(constants_1.ResourceServiceType.TDP)}/apps/${manifest.id}/app-validation/${validation.id}`);
            ...
            if (context.platform === teamsfx_api_1.Platform.CLI) {
                const message = [
                    ...
                    {
                        content: `${(0, constants_1.getResourceServiceEndpoint)(constants_1.ResourceServiceType.TDP)}/apps/${manifest.id}/app-validation/${response.appValidationId}`,
                        color: teamsfx_api_1.Colors.BRIGHT_CYAN,
                    },
                ];
                ...
                const message = (0, localizeUtils_1.getLocalizedString)("driver.teamsApp.progressBar.validateWithTestCases.step", response.status, `${(0, constants_1.getResourceServiceEndpoint)(constants_1.ResourceServiceType.TDP)}/apps/${manifest.id}/app-validation`);
    ...
    async runningBackgroundJob(args, context, appStudioToken, response, teamsAppId) {
        const validationRequestListUrl = `${(0, constants_1.getResourceServiceEndpoint)(constants_1.ResourceServiceType.TDP)}/apps/${teamsAppId}/app-validation`;
        try {
    ...
    evaluateValidationResults(args, context, resultResp, teamsAppId) {
        ...
        const validationStatusUrl = `${(0, constants_1.getResourceServiceEndpoint)(constants_1.ResourceServiceType.TDP)}/apps/${teamsAppId}/app-validation/${resultResp.appValidationId}`;
